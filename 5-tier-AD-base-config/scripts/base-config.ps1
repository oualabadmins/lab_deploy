<#  base-config.ps1
	...
	10/5/2016 Added code to add redmond\oualabadmins to local admins 
		if computer joined to Redmond
	10/27/2016 Added sections: disable IE ESC, disable server mgr, 
		disable file warnings, configure Explorer, set power mgmt, 
		set time zone to PST, set wusa to auto-update
	10/17/2018 Revised to run Office client installer using version 
		from $OfficeVersion param, and remote PS to add new SH to RDS
	10/24/2018 Added code to run Office installer using passed param 
		$OfficeVersion, RDS config if $role is RDS
	1/16/2019 Fixed issues with several sections, REM'd out 'Enable 
		RD firewall rules'
